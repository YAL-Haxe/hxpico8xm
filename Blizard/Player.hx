...
class Player {
	...
	static function pointFlag(x:Fixed, y:Fixed, f:Int) {
		return fget(mget(x / 8, y / 8), f);
	}
	
	static function pointFree(x:Fixed, y:Fixed) {
		return pointFlag(x, y, mode) || !pointFlag(x, y, 0);
	}
	...
	static function update() {
		...
		if (y >= 124) { // restart
			Main.loadLevel(Main.level);
		}
		
		var d = mget(x / 8, y / 8);
		if (d == 7) {
			// exit
			Main.loadLevel(Main.level + 1);
		}
	}
